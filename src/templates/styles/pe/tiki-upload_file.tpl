...
{/if}
	...
	<div class="button2">
            <a href="#" onclick="javascript:flip('uploadinfo{$fileInfo.fileId}');flip('uploadinfo{$fileInfo.fileId}_close','inline');return false;" class="linkbut">
                {tr}Additional Info{/tr}
		<span id="uploadinfo{$fileInfo.fileId}_close" style="display:none">({tr}Hide{/tr})</span>
	    </a>
        </div>
        <div style="display:none;" id="uploadinfo{$fileInfo.fileId}">
	    {tr}You can link to the file from a Wiki page using{/tr}: <div class="code">[tiki-download_file.php?fileId={$fileInfo.fileId}|{$fileInfo.name}]</div>
            Você pode modificar o tamanho de exibição da imagem modificando os parâmetros "y" e "x": <div class="code">&#x7bimg src="tiki-download_file.php?fileId={$fileInfo.fileId}&thumbnail&y=50&x=100" link="tiki-download_file.php?fileId={$fileInfo.fileId}&display"}</div>
            {tr}You can display an image in a Wiki page using{/tr}: <div class="code">&#x7b;img src="tiki-download_file.php?fileId={$fileInfo.fileId}&amp;thumbnail=y" link="{$fileInfo.dllink}" alt="{$fileInfo.name}"}</div>
            {tr}You can link to the file from an HTML page using{/tr}: <div class="code">&lt;a href="{$fileInfo.dllink}"&gt;{$fileInfo.name}&lt;/a&gt;</div>
        </div>
