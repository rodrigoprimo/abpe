     ...
     {/if}
        <fieldset>
          {* <legend>{tr}Login as{/tr}&hellip;</legend> *}
            <div><label class="login" for="login-user">Entrar como usuário{* {if $prefs.login_is_email eq 'y'}{tr}Email{/tr}{else}{tr}User{/tr}{/if} *}:</label>
		{if $loginuser eq ''}
	  ...
	  <script type="text/javascript">document.getElementById('login-user').focus();</script>
		...
		<script type="text/javascript">document.getElementById('login-user').focus();</script>
          ...
          {/if}
          <div><label class="login" for="login-pass">{tr}Password{/tr}:</label>
          ...
          {if $prefs.rememberme ne 'disabled'}
            ...
            {/if}
          {/if}
          <p style="text-align: left"><input type="image" src="styles/pe/entrar_btn.png" name="login"></p>
       </fieldset>
...
<img src="styles/pe/abpe.png" style="margin-top: 30px; border: 0;" />
</div>
